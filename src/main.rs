...
#[tokio::main]
async fn main() {
    // Create the in-memory storage cum db
    ...
    let get_profile = warp::path!("user" / Uuid)
        .and(warp::get())
        .and(with_db(db.clone()))
        .and_then(get_user);

    let update_profile = warp::path!("user" / Uuid)
        .and(warp::put())
        .and(json_body())
        .and(with_db(db.clone()))
        .and_then(update_user);

    let delete_profile = warp::path!("user" / Uuid)
        .and(warp::delete())
        .and(with_db(db.clone()))
...
// This wraps the database so we can inject it into handlers
fn with_db(db: Db) -> impl Filter<Extract = (Db,), Error = Infallible> + Clone {
    warp::any().map(move || db.clone())
}

// This extracts JSON body and converts into struct
fn json_body<T: DeserializeOwned + Send>() -> impl Filter<Extract = (T,), Error = warp::Rejection> + Clone {
    warp::body::content_length_limit(1024 * 16) // limit to 16 KB to prevent abuse
    .and(warp::body::json())
}
...
async  fn register_user(body: RegisterRequest, db: Db) -> Result<warp::reply::Response, warp::Rejection> {
    ...
    db.lock().unwrap().insert(user_id, user.clone());
...
// Update user profile by ID - name and password
